<!-- 
  NOTE: This reference uses an extended multi-file format (DESIGN.md + tokens.md + 
  components.md + platform-mapping.md) rather than the standard 9-module Design MD format.
  It was imported from a standalone design system skill with richer craft guidance.
  The extended format is preserved because Nothing's design philosophy (spacing-as-meaning, 
  three-layer rule, craft rules) cannot be reduced to the standard 9-module template 
  without losing critical design intent.
-->
